...
#endif
{
	static_cast<TuioClient25D*>(obj)->socket->Run();
	return 0;
};

void TuioClient25D::lockObjectList() {
	if(!connected) return;
#ifndef WIN32	
...
void TuioClient25D::unlockObjectList() {
	if(!connected) return;
#ifndef WIN32	
...
void TuioClient25D::lockCursorList() {
	if(!connected) return;
#ifndef WIN32	
...
void TuioClient25D::unlockCursorList() {
	if(!connected) return;
#ifndef WIN32	
...
TuioClient25D::TuioClient25D(int port)
: socket      (NULL)
, currentFrame(-1)
, maxCursorID (-1)
, thread      (NULL)
...
TuioClient25D::~TuioClient25D() {	
	delete socket;
}

void TuioClient25D::ProcessBundle( const ReceivedBundle& b, const IpEndpointName& remoteEndpoint) {
...
void TuioClient25D::ProcessMessage( const ReceivedMessage& msg, const IpEndpointName& remoteEndpoint) {
	try {
		...
		if( strcmp( msg.AddressPattern(), "/tuio/2Dobj" ) == 0 ){
			...
			if (strcmp(cmd,"set")==0) {	
				...
				if (!lateFrame) {
					...
					for (std::list<TuioObject25D*>::iterator tobj=objectList.begin(); tobj != objectList.end(); tobj++) {
					...
					for (std::list<TuioObject25D*>::iterator iter=frameObjects.begin(); iter != frameObjects.end(); iter++) {
						...
						switch (tobj->getTuioState()) {
							case TUIO_REMOVED:
								...
								for (std::list<TuioListener25D*>::iterator listener=listenerList.begin(); listener != listenerList.end(); listener++)
									(*listener)->removeTuioObject25D(frameObject);

								lockObjectList();								
								for (std::list<TuioObject25D*>::iterator delobj=objectList.begin(); delobj!=objectList.end(); delobj++) {
							...
							case TUIO_ADDED:
								...
								lockObjectList();
								frameObject = new TuioObject25D(currentTime,tobj->getSessionID(),tobj->getSymbolID(),tobj->getX(),tobj->getY(),tobj->getAngle());
			...
			if (strcmp(cmd,"set")==0) {	
				...
				args >> s_id >> xpos >> ypos >> xspeed >> yspeed >> zspeed >> maccel;
				...
				if (!lateFrame) {
					...
					for (std::list<TuioCursor25D*>::iterator tcur=cursorList.begin(); tcur != cursorList.end(); tcur++) {
					...
					for (std::list<TuioCursor25D*>::iterator iter=frameCursors.begin(); iter != frameCursors.end(); iter++) {
						...
						switch (tcur->getTuioState()) {
							case TUIO_REMOVED:
								...
								lockCursorList();
								for (std::list<TuioCursor25D*>::iterator delcur=cursorList.begin(); delcur!=cursorList.end(); delcur++) {
							...
							case TUIO_ADDED:
								...
								frameCursor = new TuioCursor25D(currentTime,tcur->getSessionID(),c_id,tcur->getX(),tcur->getY());
...
void TuioClient25D::ProcessPacket( const char *data, int size, const IpEndpointName& remoteEndpoint ) {
	try {
...
void TuioClient25D::connect(bool lk) {
...
void TuioClient25D::addTuioListener(TuioListener25D *listener) {
	listenerList.push_back(listener);
}

void TuioClient25D::removeTuioListener25D(TuioListener25D *listener) {
	std::list<TuioListener25D*>::iterator result = find(listenerList.begin(),listenerList.end(),listener);
	if (result!=listenerList.end()) listenerList.remove(listener);
}

TuioObject25D* TuioClient25D::getTuioObject25D(long s_id) {
	lockObjectList();
	for (std::list<TuioObject25D*>::iterator iter=objectList.begin(); iter != objectList.end(); iter++) {
...
TuioCursor25D* TuioClient25D::getTuioCursor25D(long s_id) {
	lockCursorList();
	for (std::list<TuioCursor25D*>::iterator iter=cursorList.begin(); iter != cursorList.end(); iter++) {
		if((*iter)->getSessionID()==s_id) {
			unlockCursorList();
...
std::list<TuioObject25D*> TuioClient25D::getTuioObject25Ds() {
	lockObjectList();
...
std::list<TuioCursor25D*> TuioClient25D::getTuioCursor25Ds() {
	lockCursorList();
