...
static void debug_print_matrix( int* data, int w, int h, MyBlobRect roi, int gridw, int gridh){
	int i,j, wr, hr, w2, h2;
	int d;
...
static void debug_print_matrix2(int* ids, int* data, int w, int h, MyBlobRect roi, int gridw, int gridh){
	int i,j, wr, hr, w2, h2;
	int d;
	wr = (roi.width-1) % gridw;
	hr = (roi.height-1) % gridh;
		...
		int* tree_size )
{
	...
	return find_connection_components_with_roi_coarse(data,w,h,roi,thresh,1,1,tree_size);
}
...
static Node* find_connection_components_with_roi_coarse(
		...
		int* tree_size )
{
	const int stepheight2 = stepwidth; //stepheigth will be ignored.
	//init
  ...
  int b=h-roi.y-roi.height; //bottom border
	...
	int shr = (roi.height-1)%stepheight2; // remainder of height/stepheight;
	int sh = stepheight2*w;
	int sh1 = (stepheight2-1)*w;
	int sh2 = shr*w;
...
//	int* anchors = malloc( max_comp*sizeof(int) ); //store positions of first elements with id.
	...
	unsigned char* const triangle = malloc(
			(triwidth+1)* ( (roi.height-1)/stepheight2 + 1)
...
if( stepwidth*stepheight2 >1 ){
	debug_print_matrix( ids, w, h, roi, stepwidth, stepheight2);
	//printf("\n\n");
	debug_print_matrix2( ids, comp_same, w, h, roi, stepwidth, stepheight2);
}
#endif
...
void myblob_find_blobs(Myblob *blob, const unsigned char *data, const int w, const int h, const MyBlobRect roi, const unsigned char thresh){
	...
	blob->tree = find_connection_components_with_roi_coarse(
			data, w, h, roi, thresh,
