...
int JsonConfig::setConfig(char* json_str){
	clearConfig();
	m_pjson_root = cJSON_Parse(json_str);
	return 0;
}

char* JsonConfig::getConfig(){
	return cJSON_Print(m_pjson_root);
}

int JsonConfig::loadConfigFile(char* filename, LoadDefaultsType* loadDefaultsFunc=NULL ){
		clearConfig();
...
int JsonConfig::saveConfigFile(char* filename)
{
	FILE *file;
...
{
	cJSON* root = cJSON_CreateObject();	
	cJSON_AddItemToObject(root, "type", cJSON_CreateString("mmtt_setting"));
	cJSON_AddItemToObject(root, "host", cJSON_CreateString("0.0.0.0"));
	cJSON_AddItemToObject(root, "port", cJSON_CreateString("8080"));
	cJSON_AddItemToObject(root, "lastSetting", cJSON_CreateString("defaultSetting.json"));
	return root;
}
...
cJSON* JsonConfig::loadKinectSetting()
{
	...
	cJSON_AddStringToObject(root, "type", "kinect_setting");
